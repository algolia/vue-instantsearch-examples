...
export default {
  ...
  data() {
    ...
    const state = helper.url.getStateFromQueryString(queryString);
    searchStore._helper.setState(state);

    return { searchStore };
  },
  watch: {
    'searchStore.searchParameters'(to) {
      const queryString = helper.url.getQueryStringFromState(
        this.searchStore.algoliaHelper.state
      );
